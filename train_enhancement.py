...
def train(loader, model, criterion, opt, writer, step, epoch, mixup_c = None, mixup = True, device = torch.device('cuda')):
    ...
    for idx, (lol, target) in enumerate(loop):
        ...
        if epoch > 5 and mixup is True:
            lol, target = mixup_c.augment(lol, target)
...
def validate(loader, model, criterion, writer, step, epoch, device = torch.device('cuda')):
    ...
    with torch.no_grad():
        ...
        gen_img = torch.clip(fixed_ip_valid, 0, 1)
...
def main():
    ...
    for epoch in range(100):
        ...
        with torch.no_grad():
          ...
          gen_img1 = torch.clip(gen_img1, 0, 1)
        ...
        step = train(train_loader, model, criterion, opt, writer, step, epoch, mixup_c)
        scheduler.step()
